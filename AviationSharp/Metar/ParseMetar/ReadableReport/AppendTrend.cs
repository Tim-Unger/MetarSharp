using static AviationSharp.Metar.Extensions.DistanceExtensions;
...
namespace AviationSharp.Metar.Parse.ReadableReport
{
    internal class Trend
    {
        internal static string Append(Metar metar)
        {
            ...
            foreach (AviationSharp.Metar.Trend trend in trends)
            {
                foreach (var singleTrend in trend.TrendList ?? Enumerable.Empty<object>())
                {
                    ...
                    {
                        "Visibility" => ParseVisibility((AviationSharp.Metar.Visibility)singleTrend),
                        "Weather" => ParseWeather((AviationSharp.Metar.Weather)singleTrend),
                        "Wind" => ParseWind((AviationSharp.Metar.Wind)singleTrend),
                        "Cloud" => ParseCloud((AviationSharp.Metar.Cloud)singleTrend),
        ...
        private static string ParseVisibility(AviationSharp.Metar.Visibility visibility)
        {
        ...
        private static string ParseWeather(AviationSharp.Metar.Weather weather)
        {
            var stringBuilder = new StringBuilder();
        ...
        private static string ParseWind(AviationSharp.Metar.Wind wind)
        {
        ...
        private static string ConvertWind(AviationSharp.Metar.Wind wind)
        {
        ...
        private static string ConvertGusts(AviationSharp.Metar.Wind wind)
        {
            return "\n" + $"Gusting up to {wind.WindGusts} {wind.WindUnitDecoded}";
        }

        private static string ConvertVariation(AviationSharp.Metar.Wind wind)
        {
            return "\n" + $"Variable between {wind.WindVariationLow} Degrees and {wind.WindVariationHigh} Degrees.";
        }

        private static string ParseCloud(AviationSharp.Metar.Cloud cloud)
        {
            var stringBuilder = new StringBuilder();
